...
module FetchPrecompiledGem
  ...
  def fetch_gem(spec)
    if !spec.cached_file_exists? && BundlerWrap.config['source']
      uri = URI.parse(File.join(BundlerWrap.config['source'], spec.explicit_file_name))
...
module InstallPrecompiledGem
  ...
  def install
    if !spec.cached_file_exists?
      ...
      temp_path = "/tmp/.fcpm-ruby-#{spec.name}.tmp"
...
Gem::BasicSpecification.send :prepend, CachedInstall
Bundler::Source::Rubygems.send :prepend, FetchPrecompiledGem
Bundler::GemInstaller.send :prepend, InstallPrecompiledGem
